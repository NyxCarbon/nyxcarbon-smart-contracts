...
describe("Loan Contract -- LSP7/ECR20 Token", function () {
  async function deployLoanFixture() {
    ...
    const carbonCreditsStaked: bigint = BigInt(12500);
    ...
    const hardhatLoan = await Loan.deploy(hardhatLoanTxData.target as string, hardhatRWAVerification.target as string, hardhatToken.target as string)
  ...
  describe("Deployment", function () {
    ...
    it("Should set the status of the loan as Created", async function () {
      ...
      expect(await hardhatLoanTxData.getDecodedUint256(tokenId1, _NYX_LOAN_STATUS)).to.equal(0);
    });
  ...
  describe("Fund Loan", function () {
    ...
    it("Should calculate the TLV and set currentBalance", async function () {
      const { hardhatLoan, hardhatLoanTxData, addr1, apy, initialLoanAmount, tokenId1, _NYX_LOAN_BALANCE } = await loadFixture(deployLoanFixture);
      await hardhatLoan.connect(addr1).fundLoan(tokenId1);
    ...
    it("Should set the status of the loan as Funded", async function () {
      const { hardhatLoan, hardhatLoanTxData, addr1, tokenId1, _NYX_LOAN_STATUS } = await loadFixture(deployLoanFixture);
      await hardhatLoan.connect(addr1).fundLoan(tokenId1);
  ...
  describe("Accept Loan", function () {
    ...
    it("Should calculate balance (loan + interest)", async function () {
      ...
      expect(ethers.formatEther(await hardhatLoanTxData.getDecodedUint256(tokenId1, _NYX_LOAN_BALANCE))).to.equal(((offChainTransactionFee + offChainNetMonthlyPayment) * 36).toFixed(1));
    });
    
    it("Should set the loan state to Taken", async function () {
      const { hardhatLoan, hardhatLoanTxData, addr1, addr2, tokenId1, _NYX_LOAN_STATUS } = await loadFixture(deployLoanFixture);
      await hardhatLoan.connect(addr1).fundLoan(tokenId1);
      await hardhatLoan.connect(addr2).acceptLoan(tokenId1);
  ...
  describe("Execute Swap", function () {
    it("Should execute the swap when the profit is greater than 32%", async function () {
      const { hardhatLoan, hardhatLoanTxData, addr1, addr2, tokenId1, _NYX_LOAN_BALANCE, _NYX_LOAN_STATUS } = await loadFixture(deployLoanFixture);
      await hardhatLoan.connect(addr1).fundLoan(tokenId1);
      await hardhatLoan.connect(addr2).acceptLoan(tokenId1);
      await hardhatLoan.setCarbonCreditPrice(ethers.parseEther('52.86'));
      await hardhatLoan.evaluateSwapState(tokenId1);
  ...
  describe("Make Payment", function () {
    ...
    it("Should allow borrower to transfer tokens to the lender (i.e., make a payment)", async function () {
      const { hardhatToken, hardhatLoan, hardhatLoanTxData, owner, addr1, addr2, tokenId1, initialLoanAmount, apy, transactionBPS, _NYX_LOAN_BALANCE, _NYX_PAYMENT_INDEX } = await loadFixture(deployLoanFixture);
      await hardhatLoan.connect(addr1).fundLoan(tokenId1);
      await hardhatLoan.connect(addr2).acceptLoan(tokenId1);
