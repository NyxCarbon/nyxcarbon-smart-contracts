...
describe("Loan Contract Simple -- Native Token", function () {
  ...
  describe("Execute Swap", function () {
    it("Should execute the swap when the profit is greater than 32%", async function () {
      const { hardhatLoan, hardhatLoanTxData, hardhatRWAVerification, verifiedProjectName, verifiedProjectLink, verifiedProjectUnits, verifiedProjectGeographicIdentifier, verifiedProjectVerificationLink, tokenId1, _NYX_LOAN_BALANCE, _NYX_LOAN_STATUS, _NYX_CARBON_CREDITS_BALANCE } = await loadFixture(deployLoanFixture);
      await hardhatLoan.addVerifiedProject(tokenId1, verifiedProjectName, verifiedProjectLink, verifiedProjectUnits, verifiedProjectGeographicIdentifier, verifiedProjectVerificationLink);
      await hardhatLoan.setCarbonCreditPrice(ethers.parseEther('52.86'));
      await hardhatLoan.evaluateSwapState(tokenId1);
    ...
    it("Should create multiple carbon credit NFTs when auto-swapped", async function () {
      const { hardhatLoan, hardhatLoanTxData, hardhatRWAVerification, verifiedProjectName, verifiedProjectLink, verifiedProjectUnits, verifiedProjectGeographicIdentifier, verifiedProjectVerificationLink, tokenId1, tokenId2, _NYX_LOAN_BALANCE, _NYX_LOAN_STATUS, _NYX_CARBON_CREDITS_BALANCE } = await loadFixture(deployLoanFixture);
      await hardhatLoan.addVerifiedProject(tokenId1, verifiedProjectName, verifiedProjectLink, verifiedProjectUnits, verifiedProjectGeographicIdentifier, verifiedProjectVerificationLink);
      await hardhatLoan.addVerifiedProject(tokenId1, "Remove Carbon 1", verifiedProjectLink, 250, '-1.940278, 29.873888', "https:link.com");
    ...
    it("Should emit LoanSwapped event", async function () {
      const { hardhatLoan, hardhatRWAVerification, owner, addr1, addr2, verifiedProjectName, verifiedProjectLink, verifiedProjectUnits, verifiedProjectGeographicIdentifier, verifiedProjectVerificationLink, tokenId1 } = await loadFixture(deployLoanFixture);
      await hardhatLoan.addVerifiedProject(tokenId1, verifiedProjectName, verifiedProjectLink, verifiedProjectUnits, verifiedProjectGeographicIdentifier, verifiedProjectVerificationLink);
      await hardhatLoan.setCarbonCreditPrice(ethers.parseEther('52.86'));
      await hardhatLoan.evaluateSwapState(tokenId1);
      ...
      await expect(hardhatLoan.executeSwap(tokenId1))
        ...
        .withArgs(addr1.address, tokenId1, verifiedProjectName, verifiedProjectLink, verifiedProjectUnits, verifiedProjectGeographicIdentifier, verifiedProjectVerificationLink, [await hardhatLoan.getAddress()])
        .to.emit(hardhatLoan, "LoanSwapped")
        .withArgs(25, BigInt(3215) * BigInt(1e17), 3215)
    });
